...
public class ShadowParticle extends PixelParticle.Shrinking {
	...
	public static final Emitter.Factory BLIND = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((ShadowParticle)emitter.recycle( ShadowParticle.class )).resetBlinded( x, y );
		}
	};
	...
	public static final Emitter.Factory TRAIL = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((ShadowParticle)emitter.recycle( ShadowParticle.class )).resetTrail( x, y );
		}
	};
	...
	public void resetBlinded( float x, float y ) {
		revive();

		size = 3;
		left = lifespan = 0.4f;

		speed.polar( Random.Float( PointF.PI2 ),32 /*Random.Float( 16, 32 )*/ );
		this.x = x - speed.x * lifespan;
		this.y = y - speed.y * lifespan;
	}
	...
	public void resetTrail( float x, float y ) {
		revive();

		speed.set( Random.Float( -16, +16 ), Random.Float( -16, -32 ) );
