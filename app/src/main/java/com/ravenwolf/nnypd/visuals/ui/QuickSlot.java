...
public class QuickSlot extends Button implements WndBag.Listener {
	...
	private static final String TXT_SELECT_ITEM_OFFHAND = "选择一个装备到副手的物品";
			...
			protected boolean onLongClick() {
                ...
                if (item instanceof Charm) {
                    ...
                    if (charm.canUnleash()) {
                        cancel();
                        if (item != null) {
                            item.execute(Dungeon.hero, charm.secondaryAction());
	...
	public Item select() {
        ...
        if( this == quickslot0 ) {
            ...
            value = (Dungeon.hero.belongings.weap2 instanceof RangedWeapon &&
                    ((RangedWeapon) Dungeon.hero.belongings.weap2).checkAmmo(Dungeon.hero, false) ?
                    Dungeon.hero.belongings.weap2 : Dungeon.hero.belongings.ranged instanceof ThrowingWeaponAmmo ? null : Dungeon.hero.belongings.ranged);

        } else
        if( this == quickslotX ) {

            value = Dungeon.hero.belongings.weap2;
	...
	private void useTargeting() {
        ...
        currentTarget=TagAttack.getCurrentTarget();
	...
	public static void refresh() {
        ...
        if (quickslotX != null) {
            quickslotX.item( quickslotX.select() );
        }
	...
	public static void target( Item item, Char target ) {
		if (target != Dungeon.hero) {
            ...
            quickslotX.currentTarget = target;
	...
	public static void cancel() {
        if (quickslotX != null && quickslotX.targeting) {
            quickslotX.crossB.visible = false;
            quickslotX.crossM.remove();
