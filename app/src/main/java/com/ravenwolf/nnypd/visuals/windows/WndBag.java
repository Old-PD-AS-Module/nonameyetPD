...
import com.ravenwolf.nnypd.actors.hero.Belongings;
...
import com.ravenwolf.nnypd.items.weapons.melee.MeleeWeaponLightOH;
...
public class WndBag extends WndTabbed {
	...
	public WndBag( Bag bag, Listener listener, Mode mode, String title ) {
		...
		nCols = NoNameYetPixelDungeon.landscape() ? COLS_L : COLS_P;
		nRows = NoNameYetPixelDungeon.landscape() ? ROWS_L : ROWS_P ;
//		nRows = (Belongings.BACKPACK_SIZE + 4 + 1) / nCols + ((Belongings.BACKPACK_SIZE + 4 + 1) % nCols > 0 ? 1 : 0) + ( !NoNameYetPixelDungeon.landscape() ? 1 : 0 );
		...
		bags.add( stuff.getItem( Baldric.class ) );
...
//        Ankh ankh = stuff.getItem( Ankh.class );
		...
		//placeItem( stuff.ranged != null ? stuff.ranged : new Placeholder( ItemSpriteSheet.RANGED ) );
        ...
        if (container instanceof Baldric)
			placeItem( stuff.ranged != null ? stuff.ranged : new Placeholder( ItemSpriteSheet.RANGED ) );
	...
	/*
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.FOR_SALE && (item.visible && item.price() > 0) && (!item.isEquipped( Dungeon.hero ) || !item.isCursed()) ||
						mode == Mode.UPGRADEABLE && ( item.isUpgradeable() && ( !item.isIdentified() || item.bonus < Item.MAX_BONUS_LVL ) ) ||
						mode == Mode.UPGRADEABLE_BM && ( item.isUpgradeable() && ( !item.isIdentified() || item.bonus < Item.MAX_BONUS_LVL || item instanceof BodyArmorCloth) ) ||
						...
						mode == Mode.WEAPON_ARMOR && !item.isEquipped( Dungeon.hero ) && (item instanceof BodyArmor || item instanceof MeleeWeapon)||
						...
						mode == Mode.HERB && (item instanceof Herb) ||
								mode == Mode.CURSED && (item.isCursed() || item instanceof EquipableItem && !item.isCursedKnown())  ||
						mode == Mode.ALL
					);
				}
			} else {
