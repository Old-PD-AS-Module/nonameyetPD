...
public class Lightning extends Group {
	...
	public Lightning( int[] cells, int length, Callback callback ) {
		this(cells, length, Effects.get( Effects.Type.LIGHTNING ), callback );
	}
	
	public Lightning( int[] cells, int length, Image proto,Callback callback ) {
		...
		for (int i=0; i < length; i++) {
			...
			cy[i] = (c / Level.WIDTH + 0.5f) * DungeonTilemap.SIZE-4;
		}
