...
public class EmoIcon extends Image {
	...
	public void update() {
		...
		if (visible) {
			if (growing) {
				scale.set( Math.min( maxSize, scale.x + Game.elapsed * timeScale ) );
				if (scale.x >= maxSize) {
					growing = false;
				}
			} else {
				scale.set( Math.max( 1, scale.x - Game.elapsed * timeScale ) );
				if (scale.x <= 1) {
					growing = true;
				}
			}
...
//			origin.set( 2.5f, height - 2.5f );
			...
			//added to avoid show in random position
			update();
		}
	}

    public static class Inspect extends EmoIcon {

        public Inspect( CharSprite owner ) {
