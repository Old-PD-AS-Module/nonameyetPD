...
import com.ravenwolf.nnypd.items.herbs.SungrassHerb;
...
public class GardenPainter extends Painter {
	...
	public static void paint( Level level, Room room ) {
        ...
        if (spawnedAmount < (room.width()+room.height()) /3) {
            int pos;
            do {
                pos = room.random();
            } while (level.map[pos] != Terrain.HIGH_GRASS || level.heaps.get( pos ) != null);
            level.drop(new SungrassHerb(), pos, false);
        }
