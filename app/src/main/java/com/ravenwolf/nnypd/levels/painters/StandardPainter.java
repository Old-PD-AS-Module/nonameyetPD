...
//            door.set( Room.Door.Type.HIDDEN );
        ...
        if( level.feeling == Level.Feeling.HAUNT &&	Math.min( room.width(), room.height() ) >= 4 && Random.Int( 3 ) == 0 && !Dungeon.isPathwayLvl() ) {
...
//					 Burned room
                ...
                case 4:
                    ...
                    break;
                case 5:
                    if (!Dungeon.bossLevel() && Math.min(room.width(), room.height()) >= 5 && !Dungeon.isPathwayLvl()) {
                        paintFissure(level, room);
                        return;
                    }
                    break;
	...
	private static void paintStriped( Level level, Room room ) {
        ...
        int chance= (level.feeling == Level.Feeling.GRASS ? 15:9) - Dungeon.chapter();
	...
	private static void paintBridge( Level level, Room room ) {
        ...
        if( room.connected.size() == 2 ) {
            ...
            fill( level, room.left + 1, room.top + 1, room.width() - 1, room.height() - 1 ,
                    level.feeling == Level.Feeling.WATER || Dungeon.isPathwayLvl()?
