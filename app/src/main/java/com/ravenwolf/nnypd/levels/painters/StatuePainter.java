...
import com.ravenwolf.nnypd.items.potions.PotionOfInvisibility;
...
//		int cy = c.y;
        ...
        Item prize = prize();
        if (prize instanceof MeleeWeapon && Random.Int(2) == 0){
            set(level, pos, Terrain.PEDESTAL);
            level.drop(prize, pos, true).type = Heap.Type.HEAP;
            HauntedWeaponSleep hauntedWeapon = new HauntedWeaponSleep();
            hauntedWeapon.setStats(pos, (MeleeWeapon) prize);
            level.hazards.add(hauntedWeapon);
        }else {
