...
public final class ShadowCaster {
	...
	public static void castShadow( int x, int y, boolean[] fieldOfView, int distance, boolean[] blocking ) {

		//losBlocking = flying ? Level.losBlockHigh.clone() : Level.losBlockLow.clone() ;
		losBlocking = blocking;
/*
        Blob blob = Dungeon.level.blobs.get( Darkness.class );

        if( blob != null ) {
            BArray.or(losBlocking, blob.bln, losBlocking);
        }
*/
