 ...
 */
package com.ravenwolf.nnypd.items.rings;
...
import com.ravenwolf.nnypd.items.ItemStatusHandler;
...
//	}
	...
	public boolean doEquip( final Hero hero ) {
		...
		if (hero.belongings.ring1 != null && hero.belongings.ring2 != null) {
			...
			NoNameYetPixelDungeon.scene().add(
				new WndOptions( TXT_UNEQUIP_TITLE, TXT_UNEQUIP_MESSAGE, 
					...
					protected void onSelect( int index ) {
                        ...
                        if( QuickSlot.quickslot1.value == Ring.this && !equipped.isCursed() )
                            QuickSlot.quickslot1.value = equipped ;

                        if( QuickSlot.quickslot2.value == Ring.this && !equipped.isCursed() )
                            QuickSlot.quickslot2.value = equipped ;

                        if( QuickSlot.quickslot3.value == Ring.this && !equipped.isCursed() )
            ...
            if( ( !isCursed() || isCursedKnown() || !detectCursed( this, hero ) ) ) {
...
//		}
	...
	public String desc() {
		if( isIdentified() && bonus >= 0 ) {
			return "这枚戒指_已被鉴定_。同种类戒指的效果会互相叠加。";
		} else if( isCursedKnown() && bonus < 0 ) {
			return "这枚戒指被诅咒了。其效果与非诅咒戒指正好相反，并且无法正常取下。";
		} else {
			return "这枚戒指_未被鉴定_。你并不清楚它的具体作用。";
		}
	}
	...
	public Item random() {
		...
		//if ( Random.Int( Dungeon.chapter() * 8  )+4 < Random.Int( n * 6 )+Dungeon.chapter() * 6  ) {
		 if ( Random.Int( Dungeon.chapter() * 5  )+n*Dungeon.chapter() > Random.Int( Dungeon.chapter() * 6 )+Dungeon.chapter() ) {
    ...
    public float effect( int level ) {
		return isCursed() ?  0.1f * (level-2) : 0.2f + 0.1f * level ;
    }
	...
	public class RingBuff extends Buff {
		...
		/*public int level() {
            return isCursed() ?  Ring.this.bonus -2 : Ring.this.bonus + 1 ;
        }
		*/
        public float effect() {
            //return level() >= 0 ? 0.1f + 0.1f * level() : 0.1f * level();
			//return isCursed() ?  0.1f * level() : 0.1f + 0.1f * level() ;
			return Ring.this.effect(Ring.this.bonus);
